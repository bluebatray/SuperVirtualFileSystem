...
namespace virtualfilesystem {
	...
	{
		root = std::make_shared<Directory>(root_symbol, 0LL, GetCurrentEpoch(), std::weak_ptr<Directory>{});
	...
	ErrorCode FileSystem::MakeDir(const std::string& dirName)
	{
		...
		auto newDirectory = std::make_shared<Directory>("dirName", 0LL, GetCurrentEpoch(), std::weak_ptr<Directory>(currentDirectory));
	...
	std::expected<std::shared_ptr<Directory>, ErrorCode> FileSystem::GetDirectory(const std::string& path) {
